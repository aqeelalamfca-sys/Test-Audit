...
echo "== Step 3/10: Swap, Firewall & Docker config ===================="
if [ ! -f /swapfile ]; then
...
log "UFW disabled (Hostinger cloud firewall handles access — UFW breaks Docker NAT rules)"

if [ -f "${APP_DIR}/docker/daemon.json" ]; then
  cp "${APP_DIR}/docker/daemon.json" /etc/docker/daemon.json
  log "Docker daemon.json installed (live-restore enabled)"
fi
...
docker compose build backend 2>&1 || warn "Backend build had warnings"

info "Building frontend image..."
docker compose build frontend 2>&1 || warn "Frontend build had warnings"
...
if [ "$APP_HEALTHY" = "false" ]; then
  ...
  if [ -n "$PREV_COMMIT" ] && [ "$PREV_COMMIT" != "$NEW_COMMIT" ]; then
    ...
    git reset --hard "$PREV_COMMIT" -q
    docker compose build backend frontend 2>&1 || true
...
echo "== Step 10/10: Cron, systemd, logs & verification ==============="
mkdir -p "${APP_DIR}/backups" "${APP_DIR}/logs"

if [ -f "${APP_DIR}/docker/auditwise.service" ]; then
  cp "${APP_DIR}/docker/auditwise.service" /etc/systemd/system/auditwise.service
  systemctl daemon-reload
  systemctl enable auditwise.service 2>/dev/null || true
  log "Systemd service installed (auto-start on reboot)"
fi
...
CRON_HEALTH="*/5 * * * * curl -sf http://127.0.0.1:5000/api/health > /dev/null || (cd ${APP_DIR} && docker compose up -d backend 2>/dev/null)"
CRON_CERTBOT="0 4 * * 1 certbot renew --quiet --deploy-hook 'cd ${APP_DIR} && cp /etc/letsencrypt/live/${DOMAIN}/fullchain.pem nginx/ssl/ && cp /etc/letsencrypt/live/${DOMAIN}/privkey.pem nginx/ssl/ && docker compose up -d --force-recreate nginx' 2>/dev/null"
CRON_DOCKER_PRUNE="0 4 * * 0 docker image prune -af --filter 'until=168h' 2>/dev/null"

(crontab -l 2>/dev/null | grep -v "auditwise-db pg_dump" | grep -v "backups/.*sql.gz.*tail" | grep -v "api/health.*docker compose" | grep -v "certbot renew" | grep -v "docker image prune"; echo "$CRON_BACKUP"; echo "$CRON_PRUNE"; echo "$CRON_HEALTH"; echo "$CRON_CERTBOT"; echo "$CRON_DOCKER_PRUNE") | crontab -
...
if [ "$HEALTH_OK" -ge 1 ] && [ "$NGINX_CODE" = "200" ]; then
  ...
  echo "    docker compose up -d backend         # recreate backend"
  echo "    docker compose up -d --build         # rebuild & start all"
  echo "    sudo bash deploy.sh                  # full redeploy"
  echo ""
  echo "  NEVER run: iptables -F, docker compose restart, ufw enable"
  echo "================================================================"
else
  echo "================================================================"
