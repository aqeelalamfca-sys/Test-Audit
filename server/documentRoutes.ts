...
router.post("/upload", requireAuth, upload.single("file"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (useS3()) {
      const s3Key = generateStorageKey("documents", file.originalname, firmId);
      await uploadFile(file.path, s3Key, file.mimetype);
      storagePath = s3Key;
    ...
    console.log(`[UPLOAD] File uploaded: ${document.fileName} (${document.fileSize} bytes)${useS3() ? ' [S3]' : ' [local]'}`);
...
router.get("/download/:documentId", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.log(`[DOWNLOAD] Downloading file: ${document.fileName}${useS3() ? ' [S3]' : ' [local]'}`);

    const storageFile = await getFile(document.filePath);

    res.setHeader('Content-Type', document.fileType || 'application/octet-stream');
    res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(document.fileName)}"`);
    if (storageFile.contentLength) {
      res.setHeader('Content-Length', storageFile.contentLength.toString());
    }
    res.setHeader('Cache-Control', 'no-cache');
    
    storageFile.stream.on('error', (error) => {
