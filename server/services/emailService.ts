...
function getSESClient(): SESClient | null {
  ...
  console.log(`[Email] AWS SES configured (region: ${region})`);
  return sesClient;
}
...
const SENDER_EMAIL = process.env.AWS_SES_FROM_EMAIL || process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER || "noreply@auditwise.tech";

async function sendViaSES(to: string, subject: string, html: string): Promise<{ sent: boolean; message: string }> {
  const client = getSESClient();
  if (!client) {
    return { sent: false, message: "SES client not available" };
  }
  ...
  try {
    ...
    return { sent: true, message: `Email sent to ${to} via SES` };
  } catch (error: any) {
    console.error(`[Email] SES send failed to ${to}:`, error.message);
    return { sent: false, message: `SES email failed: ${error.message}` };
  }
}

async function sendViaSMTP(to: string, subject: string, html: string): Promise<{ sent: boolean; message: string }> {
  const transport = getTransporter();

  if (!transport) {
    console.log(`[Email] Email to ${to} — no transport configured, skipped.`);
  ...
  try {
    ...
    console.error(`[Email] SMTP send failed to ${to}:`, error.message);
...
export async function sendInvoiceEmail(data: InvoiceEmailData): Promise<{ sent: boolean; message: string }> {
  ...
  if (useSES()) {
    return sendViaSES(data.firmEmail, subject, html);
  }

  const transport = getTransporter();

  if (!transport) {
    console.log(`[Email] Invoice ${data.invoiceNo} for ${data.firmName} (${data.firmEmail}) — no transport configured, email skipped.`);
    console.log(`[Email] Subject: ${subject}`);
    return { sent: false, message: "No email transport configured. Invoice created but email not sent." };
  }
