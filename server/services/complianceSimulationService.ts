...
async function runISACoverageCheck(engagementId: string): Promise<SimulationSection> {
  ...
  const allRefs = [
    ...workpapers.map((w: any) => w.workpaperRef || w.section || "").filter(Boolean),
    ...checklistItems.map((a: any) => a.isaReference || a.title || "").filter(Boolean),
  ];
  ...
  for (const isa of requiredISAs) {
    ...
    if (allRefsJoined.includes(codeNum) || allRefsJoined.includes(isa.code.toUpperCase())) {
...
async function runEngagementFileReview(engagementId: string): Promise<SimulationSection> {
  ...
  const reviewedSignOffs = signOffs.filter((s: any) => s.reviewedById || s.partnerApprovedById);
  ...
  const criticalNotes = reviewNotes.filter((n: any) => n.severity === "CRITICAL");
  ...
  checks++;
  const lateReviews = signOffs.filter((s: any) => {
...
async function runISQMStressTest(engagementId: string): Promise<SimulationSection> {
  ...
  if (!engagement) {
    return { name: "ISQM Stress Test", status: "SKIPPED", findings: [], score: 0, maxScore };
  }
  ...
  checks++;
  const independenceDeclarations = await prisma.independenceDeclaration.findMany({
    where: { engagementId },
...
async function runSecuritySimulation(engagementId: string): Promise<SimulationSection> {
  ...
  if (!engagement) {
    return { name: "Security Simulation", status: "SKIPPED", findings: [], score: 0, maxScore };
  }
  ...
  const crossFirmMembers = team.filter((t: any) => t.user.firmId !== engagement.firmId);
  ...
  checks++;
  const secSignOffs = await prisma.sectionSignOff.findMany({
    where: { engagementId },
    include: { partnerApprovedBy: { select: { id: true, role: true } } },
  }).catch(() => []);

  const escalatedSignOffs = secSignOffs.filter((s: any) => {
...
async function runAIGovernanceCheck(engagementId: string): Promise<SimulationSection> {
  ...
  const unreviewed = aiLogs.filter(
    (log: any) => log.action !== "ACCEPT" && log.action !== "REJECT" && log.action !== "EDIT"
  );
  ...
  if (engagement) {
    ...
    if (lockedPhases.length > 0 && aiLogs.length > 0) {
      ...
      const recentAIInLockedPhase = aiLogs.filter(
        (log: any) => new Date(log.createdAt) > lockCheckDate
      );
  ...
  checks++;
  const acceptedWithoutEdit = aiLogs.filter((log: any) => log.action === "ACCEPT" && !log.editedOutput);
  const totalAccepted = aiLogs.filter((log: any) => log.action === "ACCEPT" || log.action === "EDIT");
...
export async function runComplianceSimulation(engagementId: string): Promise<SimulationResult> {
  ...
  const overallMaxScore = sections.reduce((sum, s) => sum + s.maxScore, 0);
  const overallPct = overallMaxScore > 0 ? overallScore / overallMaxScore : 0;
  const overallStatus: "PASS" | "FAIL" | "WARNING" =
    overallPct >= 0.8 ? "PASS" : overallPct >= 0.5 ? "WARNING" : "FAIL";
