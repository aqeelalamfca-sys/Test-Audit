...
- **FIRM_ADMIN Role Restriction**: FIRM_ADMIN cannot create or promote users to ADMIN or FIRM_ADMIN roles. This is enforced on both `/api/users` and `/api/tenant/users` endpoints, and both `user-management.tsx` and `firm-users.tsx` dropdowns hide these roles. Only Super Admin (via platform invite) can assign ADMIN/FIRM_ADMIN.
...
- **Platform Audit Logging**: All write actions logged to `PlatformAuditLog` via `server/services/platformAuditService.ts`. Two log views: Global (all platform activity with user/firm enrichment) and Firm-wise (grouped by firm with activity counts). Filters: user, action, firm, date range. Pagination support. User names/emails/roles resolved from User table. Firm names resolved from Firm relation. Platform endpoint: `/api/platform/audit-logs` (with query params), `/api/platform/audit-logs/firms-summary`, `/api/platform/audit-logs/users`. Tenant endpoint: `/api/tenant/audit-logs` (with user/date filters), `/api/tenant/audit-logs/users`.
...
- **Guide Images**: Moved from `attached_assets/` (Replit-only) to `client/public/guide-images/` (Vite public directory). Referenced as `/guide-images/...` URL paths in `client/src/lib/guide-screenshots.ts`. No `@assets` imports remain — Docker builds never fail on missing attached_assets.
- **Deploy Scripts**: `deploy.sh` (canonical idempotent deploy: system deps, Docker, swap, firewall, code, secrets, build, NGINX, SSL, backups, verification — prints LIVE only on success), `scripts/deploy.sh` (alias), `deploy/vps-update.sh` (pull + rebuild + verify homepage + health), `deploy/backup.sh` (daily Postgres dump with 30-day retention), `scripts/healthcheck.sh` (10-point endpoint verification: /, /health, SPA routes, /api), `scripts/rollback.sh` (revert N commits + backup + rebuild + verify), `scripts/verify.sh` (PASS/FAIL for Docker, local, HTTPS, DB), `scripts/diagnose.sh` (build logs + app logs + resource usage)
- **DEPLOYMENT.md**: Comprehensive deployment guide with architecture diagram, quick deploy, manual setup, endpoint table + example responses, 15-item troubleshooting matrix (symptom→cause→fix), build process diagram, verification commands
