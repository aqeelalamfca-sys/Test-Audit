      ...
      <div className="w-full px-4 py-2 space-y-4">
        ...
        {!canProceedToFinalization && totalHeads > 0 && (
          <Alert variant="destructive" data-testid="alert-finalization-blocked">
            ...
            <AlertDescription>
              {!allPlanningPrerequisitesMet
                ? "Planning prerequisites are not fully met. Complete all planning requirements before execution."
                : `Cannot proceed to Finalization — ${totalHeads - completedHeads} FS Head(s) are not yet approved. Complete all FS Head working papers.`
              }
            </AlertDescription>
          </Alert>
        )}

        <Card data-testid="card-prerequisites-panel">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                <Shield className="h-4 w-4 text-primary" />
                <CardTitle className="text-base">Planning Phase Prerequisites</CardTitle>
                <Badge
                  variant={allPlanningPrerequisitesMet ? "default" : "secondary"}
                  className={allPlanningPrerequisitesMet ? "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300" : "bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300"}
                  data-testid="badge-prerequisites-count"
                >
                  {prerequisitesPassedCount}/{planningPrerequisites.length} Passed
                </Badge>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {planningPrerequisites.map((prereq) => (
                <div
                  key={prereq.id}
                ...
                >
                  {prereq.status ? (
                    <CheckCircle2 className="h-4 w-4 text-green-600 flex-shrink-0" data-testid={`icon-check-${prereq.id}`} />
                  ) : (
                    <XCircle className="h-4 w-4 text-red-600 flex-shrink-0" data-testid={`icon-x-${prereq.id}`} />
                  )}
                  <div className="min-w-0">
                    <p className={`text-xs font-medium truncate ${prereq.status ? "text-green-700 dark:text-green-300" : "text-red-700 dark:text-red-300"}`}>
                      {prereq.label}
                    </p>
                    <p className="text-xs text-muted-foreground truncate">{prereq.isaReference}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {isaCompliance && totalHeads > 0 && (
          <Card data-testid="card-isa-compliance-summary">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  <Shield className="h-4 w-4 text-primary" />
                  <CardTitle className="text-base">ISA Compliance Status</CardTitle>
                  <Badge
                    variant={complianceSummary?.canProceedToFinalization ? "default" : "secondary"}
                    className={complianceSummary?.canProceedToFinalization
                      ? "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300"
                      : "bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300"}
                    data-testid="badge-compliance-status"
                  >
                    {complianceSummary?.canProceedToFinalization ? "All Clear" : "Action Required"}
                  </Badge>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <ComplianceItem
                  passed={isaCompliance.allHeadsHaveEvidence}
                  label="Evidence (ISA 500)"
                  sublabel={isaCompliance.allHeadsHaveEvidence ? "All attached" : "Missing"}
                  testId="compliance-evidence"
                />
                <ComplianceItem
                  passed={isaCompliance.allConclusionsWritten}
                  label="Documentation (ISA 230)"
                  sublabel={isaCompliance.allConclusionsWritten ? "All written" : "Incomplete"}
                  testId="compliance-conclusions"
                />
                <ComplianceItem
                  passed={isaCompliance.allRisksLinked}
                  label="Risk Response (ISA 330)"
                  sublabel={isaCompliance.allRisksLinked ? "All linked" : "Gaps found"}
                  testId="compliance-risks"
                />
                <ComplianceItem
                  passed={isaCompliance.allReviewPointsCleared}
                  label="Review Points"
                  sublabel={isaCompliance.allReviewPointsCleared ? "All cleared" : "Open items"}
                  testId="compliance-review-points"
                />
              </div>
              {complianceSummary?.finalizationBlockers && complianceSummary.finalizationBlockers.length > 0 && (
                <div className="mt-3 pt-3 border-t">
                  <p className="text-xs font-medium text-muted-foreground mb-1">Finalization Blockers:</p>
                  <ul className="text-xs text-muted-foreground space-y-0.5">
                    {complianceSummary.finalizationBlockers.map((b: string, i: number) => (
                      <li key={i} className="flex items-center gap-1" data-testid={`blocker-${i}`}>
