...
export default function PrePlanning() {
  ...
  useEffect(() => {
    ...
    const loadSavedData = async () => {
      try {
        ...
        if (saved.setupData) setSetupData(prev => {
          const { clientName, engagementCode, fiscalYear, status, ...rest } = saved.setupData;
          return {
            ...prev,
            ...rest,
            clientName: clientName || prev.clientName || "",
            engagementCode: engagementCode || prev.engagementCode || "",
  ...
  useEffect(() => {
    ...
    setSetupData(prev => ({
      ...
      engagementCode: engagement?.engagementCode || prev.engagementCode || "",
      fiscalYear: engagement?.fiscalYearEnd || prev.fiscalYear || "",
      status: engagement?.status || prev.status || "DRAFT",
    }));
  }, [client, engagement]);
