...
export function usePreplanningSaveBridge(
  ...
  const doSaveRef = useRef<(asDraft: boolean, silent?: boolean) => Promise<boolean>>(() => Promise.resolve(false));
  ...
  const doSave = useCallback(async (asDraft: boolean, silent = false): Promise<boolean> => {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        queryClient.invalidateQueries({ queryKey: [`/api/workspace/${engagementId}/pre-planning`] });
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && dataLoadedRef.current && !isSavingRef.current) {
        const payload = buildPayloadRef.current();
        const eid = engagementId;
        if (eid) {
          flushDraftSave("POST", `/api/workspace/${eid}/pre-planning/draft`, {
            data: payload,
            entityType: "pre-planning",
            pageKey: "pre-planning",
            engagementId: eid,
