...
export interface EQCRSavePayload {
  [key: string]: any;
}

export interface EQCRSaveBridgeResult {
  isDirty: boolean;
  isSaving: boolean;
  status: SaveStatus;
...
export function useEQCRSaveBridge(
  ...
  const sectionKey = `eqcr-${engagementId || "unknown"}`;
  ...
  const saveMutation = useCallback(async (isDraft: boolean, silent?: boolean): Promise<boolean> => {
    ...
    try {
      ...
      const endpoint = isDraft 
        ? `/api/workspace/${engagementId}/eqcr/draft`
        : `/api/workspace/${engagementId}/eqcr`;
      ...
      if (result.success !== false) {
        ...
        if (!silent) {
          toast({
            ...
            description: result.message || "Your EQCR data has been saved successfully.",
          });
        }
        
        queryClient.invalidateQueries({ 
          queryKey: [`/api/workspace/${engagementId}/eqcr`] 
        });
      ...
      if (!silent) {
        toast({
          title: "Save Failed",
          description: error.message || "Failed to save EQCR data",
          variant: "destructive",
        });
      }
      return false;
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && !isSavingRef.current && dataLoadedRef.current && engagementId) {
        const payload = buildPayloadRef.current();
        const endpoint = `/api/workspace/${engagementId}/eqcr/draft`;
