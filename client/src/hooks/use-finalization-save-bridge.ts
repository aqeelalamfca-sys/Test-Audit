...
export interface FinalizationSavePayload {
  [key: string]: any;
}

export interface FinalizationSaveBridgeResult {
  isDirty: boolean;
  isSaving: boolean;
  status: SaveStatus;
...
export function useFinalizationSaveBridge(
  ...
  const sectionKey = `finalization-${engagementId || "unknown"}`;
  ...
  const saveMutation = useCallback(async (isDraft: boolean, silent?: boolean): Promise<boolean> => {
    ...
    try {
      ...
      const endpoint = isDraft 
        ? `/api/workspace/${engagementId}/finalization/draft`
        : `/api/workspace/${engagementId}/finalization`;
      
      const method = isDraft ? "POST" : "PUT";
      const response = await apiRequest(method, endpoint, {
        data: payload,
        entityType: "finalization",
        pageKey: "finalization",
        engagementId,
        isDraft
      });
      ...
      if (result.success !== false) {
        ...
        if (!silent) {
          toast({
            ...
            description: result.message || "Your finalization data has been saved successfully.",
          });
        }
        
        queryClient.invalidateQueries({ 
          queryKey: [`/api/workspace/${engagementId}/finalization`] 
        });
      ...
      if (!silent) {
        toast({
          title: "Save Failed",
          description: error.message || "Failed to save finalization data",
          variant: "destructive",
        });
      }
      return false;
  ...
  useEffect(() => {
    ...
    if (currentSignature === lastSavedSignatureRef.current) {
      ...
      autoSaveTimerRef.current = setTimeout(() => {
        if (isDirtyRef.current && !isSavingRef.current && dataLoadedRef.current) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && !isSavingRef.current && dataLoadedRef.current && engagementId) {
        const payload = buildPayloadRef.current();
        const endpoint = `/api/workspace/${engagementId}/finalization/draft`;
