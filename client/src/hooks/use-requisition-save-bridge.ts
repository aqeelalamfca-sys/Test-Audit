...
export interface RequisitionSaveBridgeResult {
  isDirty: boolean;
  isSaving: boolean;
  status: SaveStatus;
...
export function useRequisitionSaveBridge(
  ...
  const sectionKey = `requisition-${engagementId || "unknown"}`;
  ...
  const saveMutation = useCallback(async (isDraft: boolean, silent?: boolean): Promise<boolean> => {
    ...
    try {
      ...
      const endpoint = isDraft 
        ? `/api/workspace/${engagementId}/requisition/draft`
        : `/api/workspace/${engagementId}/requisition`;
      ...
      if (result.success !== false) {
        ...
        if (!silent) {
          toast({
            ...
            description: result.message || "Your requisition data has been saved successfully.",
          });
        }
        
        queryClient.invalidateQueries({ 
          queryKey: [`/api/workspace/${engagementId}/requisition`] 
        });
      ...
      if (!silent) {
        toast({
          title: "Save Failed",
          description: error.message || "Failed to save requisition data",
          variant: "destructive",
        });
      }
      return false;
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && !isSavingRef.current && dataLoadedRef.current && engagementId) {
        const payload = buildPayloadRef.current();
        const endpoint = `/api/workspace/${engagementId}/requisition/draft`;
