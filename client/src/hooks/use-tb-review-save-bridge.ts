...
export interface TBReviewSavePayload {
  [key: string]: any;
}

export interface TBReviewSaveBridgeResult {
  isDirty: boolean;
  isSaving: boolean;
  status: SaveStatus;
...
export function useTBReviewSaveBridge(
  ...
  const sectionKey = `tb-review-${engagementId || "unknown"}`;
  ...
  const saveMutation = useCallback(async (isDraft: boolean, silent?: boolean): Promise<boolean> => {
    ...
    try {
      ...
      const endpoint = isDraft 
        ? `/api/workspace/${engagementId}/trial-balance/draft`
        : `/api/workspace/${engagementId}/trial-balance`;
      
      const method = isDraft ? "POST" : "PUT";
      const response = await apiRequest(method, endpoint, {
        data: payload,
        entityType: "trial-balance",
        pageKey: "trial-balance",
        engagementId,
        isDraft
      });
      ...
      if (result.success !== false) {
        ...
        if (!silent) {
          toast({
            ...
            description: result.message || "Your trial balance data has been saved successfully.",
          });
        }
        
        queryClient.invalidateQueries({ 
          queryKey: [`/api/workspace/${engagementId}/trial-balance`] 
        });
      ...
      if (!silent) {
        toast({
          title: "Save Failed",
          description: error.message || "Failed to save trial balance data",
          variant: "destructive",
        });
      }
      return false;
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && !isSavingRef.current && engagementId) {
        const payload = buildPayloadRef.current();
        const endpoint = `/api/workspace/${engagementId}/trial-balance/draft`;
