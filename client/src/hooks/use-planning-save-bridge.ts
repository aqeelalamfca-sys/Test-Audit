...
export function usePlanningSaveBridge(
  ...
  const saveMutationRef = useRef<(isDraft: boolean, silent?: boolean) => Promise<boolean>>(async () => false);
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && dataLoadedRef.current && !isSavingRef.current && engagementIdRef.current) {
        const payload = buildPayloadRef.current();
        const endpoint = `/api/workspace/${engagementIdRef.current}/planning/draft`;
