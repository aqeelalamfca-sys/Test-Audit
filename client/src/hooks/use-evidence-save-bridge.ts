...
export interface EvidenceSavePayload {
  [key: string]: any;
}

export interface EvidenceSaveBridgeResult {
  isDirty: boolean;
  isSaving: boolean;
  status: SaveStatus;
  ...
  const doSave = useCallback(async (asDraft: boolean): Promise<boolean> => {
    ...
    try {
      ...
      const endpoint = asDraft 
        ? `/api/workspace/${engagementId}/evidence/draft`
        : `/api/workspace/${engagementId}/evidence`;
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/workspace/${engagementId}/evidence`] });
      
      toast({
        title: asDraft ? "Draft Saved" : "Progress Saved",
        description: `Evidence vault ${asDraft ? "draft" : "progress"} saved successfully.`,
      });
      ...
      toast({
        title: "Save Failed",
        description: "Failed to save evidence vault. Please try again.",
        variant: "destructive",
      });
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDirtyRef.current && !isSavingRef.current && engagementIdRef.current) {
        ...
        flushDraftSave("POST", `/api/workspace/${eid}/evidence/draft`, {
          data: payload,
          entityType: "evidence",
          pageKey: "evidence",
          engagementId: eid,
