...
import { Bell, Settings, LogOut, User, Building2, Calendar, ChevronDown, Plus, FileText, BookOpen, MessageSquare, CheckCircle2, UserPlus, Check } from "lucide-react";
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-8 gap-2 px-2">
              ...
              <div className="hidden md:flex flex-col items-start">
                <span className="text-xs font-medium leading-tight">{user?.fullName || "User"}</span>
                <span className={`text-[10px] leading-tight ${theme.avatarText || "text-muted-foreground"}`}>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <Link href="/review-notes">
              <DropdownMenuItem>
                ...
                {unreadCount > 0 && (
                  <Badge className="ml-auto h-5 min-w-5 flex items-center justify-center p-0 text-[10px] bg-red-500">
                    {unreadCount > 9 ? "9+" : unreadCount}
                  </Badge>
                )}
              </DropdownMenuItem>
